...
const SCHEMA = z.object({
  ...
  statsRefreshAt: z.number().default(0),
  statsRangeDays: z.number().default(30),
  stats: z.any(),
});

export const name = 'llm-guardian';
export const inject = ['timer', 'llm', 'settings', 'credentials', 'sessionQuery', 'webServer'];
...
export function apply(ctx) {
  ...
  // ---------------- 用量统计聚合 ----------------
  // per-session 增量缓存：会话日志 append-only，events.length 单调增，可作签名。
  // models 的值同时记录 token 与 assistant 消息数，供模型维度准确筛选。
  const sessionCache = new Map(); // sid -> { size, days: Map<dateKey, {t, m, models: Map<model, {t, m}>}> }
  ...
  async function aggregateSession(header) {
    const sid = String(header.id);
    const snapshot = await sessionQuery.readSession(header.id);
    const events = snapshot.events ?? [];
    const cached = sessionCache.get(sid);
    if (cached && cached.size === events.length) return;
    const days = new Map();
    for (const ev of events) {
      const data = ev.data ?? {};
      if (ev.type === 'assistant/message') {
        const src = data.message && data.message.source;
        const model = src && src.kind === 'model' && src.model ? String(src.model) : 'unknown';
        const provider = src && src.provider ? String(src.provider) : '';
        const tokens = tokensOf(data.usage);
        const key = dayKey(ev.time);
        let day = days.get(key);
        if (!day) { day = { t: 0, m: 0, models: new Map() }; days.set(key, day); }
        day.t += tokens;
        day.m += 1;
        const modelStat = day.models.get(model) ?? { t: 0, m: 0, p: '' };
        modelStat.t += tokens;
        modelStat.m += 1;
        if (provider) modelStat.p = provider;
        day.models.set(model, modelStat);
    ...
    sessionCache.set(sid, { size: events.length, days });
  }

  function summarize(rangeDays) {
    const now = new Date();
    const todayStart = new Date(now.getFullYear(), now.getMonth(), now.getDate()).getTime();
    const cutoff = todayStart - (rangeDays - 1) * DAY_MS;
    const allDays = new Map();
    const winDays = new Map();
    const byModel = new Map();
    const activeSessions = new Set();
    for (const [sid, cache] of sessionCache) {
      let inWindow = false;
      for (const [key, day] of cache.days) {
        const a = allDays.get(key) ?? { t: 0, m: 0, models: new Map() };
        a.t += day.t; a.m += day.m;
        for (const [model, modelStat] of day.models) {
          const aggregate = a.models.get(model) ?? { t: 0, m: 0, p: '' };
          aggregate.t += modelStat.t;
          aggregate.m += modelStat.m;
          if (modelStat.p) aggregate.p = modelStat.p;
          a.models.set(model, aggregate);
        }
        allDays.set(key, a);
        if (dayTs(key) >= cutoff) {
          inWindow = true;
          const w = winDays.get(key) ?? { t: 0, m: 0, models: new Map() };
          w.t += day.t; w.m += day.m;
          winDays.set(key, w);
          for (const [model, modelStat] of day.models) {
            const dailyModel = w.models.get(model) ?? { t: 0, m: 0 };
            dailyModel.t += modelStat.t;
            dailyModel.m += modelStat.m;
            w.models.set(model, dailyModel);
            const aggregate = byModel.get(model) ?? { t: 0, m: 0, sessions: new Set(), p: '' };
            aggregate.t += modelStat.t;
            aggregate.m += modelStat.m;
            aggregate.sessions.add(sid);
            if (modelStat.p) aggregate.p = modelStat.p;
            byModel.set(model, aggregate);
          }
        }
      }
      if (inWindow) activeSessions.add(sid);
    }
    // 连续活跃天数：今天活跃则从今天起，否则从昨天起，逐日回溯
    let streak = 0;
    let cursor = todayStart;
    if (!allDays.has(dayKey(cursor))) cursor -= DAY_MS;
    while (allDays.has(dayKey(cursor))) { streak += 1; cursor -= DAY_MS; }

    let totalTokens = 0;
    let messages = 0;
    const days = [...winDays.entries()]
      .map(([date, v]) => ({
        date,
        tokens: v.t,
        messages: v.m,
        models: [...v.models.entries()]
          .map(([model, modelStat]) => ({ model, tokens: modelStat.t, messages: modelStat.m }))
          .sort((a, b) => b.tokens - a.tokens),
      }))
      .sort((a, b) => (a.date < b.date ? -1 : 1));
    const heatmapCutoff = todayStart - 279 * DAY_MS;
    const heatmapDays = [...allDays.entries()]
      .filter(([date]) => dayTs(date) >= heatmapCutoff)
      .map(([date, value]) => ({
        date,
        tokens: value.t,
        messages: value.m,
      ...
      .sort((a, b) => (a.date < b.date ? -1 : 1));
    for (const d of days) { totalTokens += d.tokens; messages += d.messages; }
    const models = [...byModel.entries()]
      .map(([model, aggregate]) => ({
        model,
        tokens: aggregate.t,
        messages: aggregate.m,
        sessions: aggregate.sessions.size,
        ...(aggregate.p ? { provider: aggregate.p } : {}),
      }))
      .sort((a, b) => b.tokens - a.tokens);
    return {
      ...
      topModel: models[0]
        ? {
          ...
          share: totalTokens > 0 ? models[0].tokens / totalTokens : 0,
        }
        : null,
      days,
      heatmapDays,
  ...
  async function runStats() {
    ...
    if (statsRunning) { statsPending = true; return; }
    statsRunning = true;
    try {
      const range = cfg.statsRangeDays ?? 30;
      const sessions = await sessionQuery.listSessions();
      for (const record of sessions) {
        try {
          await aggregateSession(record.header);
        } catch { /* 单个会话失败不影响整体 */ }
      }
      const snapshot = summarize(range);
      await ctx.settings.mutate(NS, [{ op: 'set', path: ['stats'], value: snapshot }]);
    } catch {
      /* 聚合失败：保留旧快照，下次刷新重试 */
    } finally {
      statsRunning = false;
      if (statsPending) { statsPending = false; runStats(); }
    }
  }

  ctx.effect(() => scope.watch((next, prev) => {
    syncConfig();
    const prevRefresh = prev ? prev.statsRefreshAt : undefined;
    const prevRange = prev ? prev.statsRangeDays : undefined;
    if (next.statsRefreshAt !== prevRefresh || next.statsRangeDays !== prevRange) {
      runStats();
    }
  }));
  ...
  // 旧版本只在“本地统计”页主动刷新时生成快照，Provider 弹窗可能一直
  // 读取缺少 heatmapDays / 日模型明细的旧结构。启动后立即重聚合一次，
  // 并定期补齐新会话数据，让总量与按天图表保持同一份数据源。
  void runStats();
  void runHealthCheck();
  ctx.effect(() => {
  ...
  ctx.effect(() => {
    const dispose = ctx.interval(runStats, 60_000);
    return () => dispose();
  });
