      ...
      planName: response.planName || ""
		...
		function usageTiers(result) {
			...
			try {
				...
				const source = Array.isArray(extra.tiers)
					? extra.tiers
					: Array.isArray(extra.windows)
						? extra.windows
						: Array.isArray(extra.quotaWindows)
							? extra.quotaWindows
							: [];
				return source.map(normalizeUsageTier).filter(Boolean);
			} catch {
				return [];
			}
		}
		...
		function tierShortName(name) {
			...
			if (["5h", "5_hour", "five_hour", "5_小时窗口"].includes(key)) return "5h";
			if (["7d", "7_day", "seven_day", "weekly_limit", "每周窗口"].includes(key)) return "7d";
			return text;
		}
		...
		function normalizeUsageTier(tier) {
			...
			let used = number(tier.utilization);
			if (used === undefined) used = number(tier.used);
			let remaining = number(tier.remaining);
			if (total !== undefined && total > 0 && total !== 100) {
				if (used !== undefined && number(tier.utilization) === undefined) used = used / total * 100;
				if (remaining !== undefined) remaining = remaining / total * 100;
			}
			used = clampPercent(used);
			remaining = clampPercent(remaining);
			if (remaining === undefined && used !== undefined) remaining = 100 - used;
			if (used === undefined && remaining !== undefined) used = 100 - remaining;
			if (used === undefined && remaining === undefined) return null;
			return {
				name: String(tier.name || tier.label || tier.window || "").trim(),
				used: used,
				remaining: remaining,
				total: 100,
				nextResetTime: resetTimestamp(
					tier.nextResetTime ?? tier.resetsAt ?? tier.resetAt ?? tier.reset_time,
				),
			};
		}

		function countdownText(resetAt, now) {
			const timestamp = resetTimestamp(resetAt);
			if (timestamp === undefined) return "";
			const diff = timestamp - (number(now) ?? Date.now());
			...
			if (hours > 0) return hours + "小时" + (minutes > 0 ? minutes + "分" : "") + "后重置";
			return totalMinutes + "分钟后重置";
		}

		function useQuotaClock(tiers) {
			const signature = tiers.map(tier => tier.nextResetTime || 0).join(":");
			const [now, setNow] = react.useState(Date.now());
			react.useEffect(() => {
				if (!tiers.some(tier => tier.nextResetTime)) return undefined;
				setNow(Date.now());
		...
		function remainingTone(remaining) {
			...
			if (value <= 30) return "var(--dsw-alias-state-warn-label)";
		...
		function UsageInline(props) {
			...
			const succeeded = usageSucceeded(result);
			const tiers = succeeded ? usageTiers(result) : [];
			const now = useQuotaClock(tiers);
			...
			if (tiers.length > 0) {
				tiers.forEach(function (tier, index) {
					...
					const countdown = countdownText(tier.nextResetTime, now);
					parts.push(h("span", {
						key: label + ":" + index,
						title: (tier.name || "套餐额度") + " · 剩余 " + fmt(tier.remaining) + "%"
							+ (tier.nextResetTime ? " · " + countdown + "（" + fmtTime(tier.nextResetTime) + "）" : ""),
						style: { color: remainingTone(tier.remaining), whiteSpace: "nowrap" },
					}, label + " 剩余 " + fmt(tier.remaining) + "%" + (countdown ? " · " + countdown : "")));
				});
			...
			return h("span", {
				style: { display: "inline-flex", alignItems: "center", flexWrap: "wrap", columnGap: "4px", rowGap: "2px" },
			}, children);
		}
			...
			return h("div", { style: C.usageTierGrid }, tiers.map(function (tier, index) {
				...
				return h("div", { key: String(tier.name || index), style: C.usageTier },
					h("div", { style: C.usageTierHead },
						h("strong", null, tierShortName(tier.name) || "套餐额度"),
						h("span", {
							style: Object.assign({}, C.meta, { color: remainingTone(remaining), fontWeight: 600 }),
						}, "剩余 " + fmt(remaining) + "%"),
					),
					...
					h("span", {
						style: C.meta,
						title: tier.nextResetTime ? "重置时间 " + fmtTime(tier.nextResetTime) : "",
					}, countdown || "已用 " + fmt(used) + "%"),
				);
			}));
		}
